    ...
    <x-admin-card title="All Users">
        ...
        <x-admin-table 
            :headers="[
                ...
                ['label' => 'Role', 'slot' => function($user) {
                    return $user->roles->pluck('name')->map(fn($role) => ucfirst($role))->join(', ') ?? '-';
                }],
                ['label' => 'Outlet', 'slot' => function($user) {
                    return optional($user->outlet)->name ?? '-';
                }],
