<div x-data="{ open: false, currentId: null, record: {} }"
    ...
    <x-admin-modal id="edit-user-modal" title="Edit User" size="lg">
        <form method="POST" :action="'{{ url('/admin/users') }}/' + currentId">
            @csrf
            @method('PUT')
            
            <x-admin-form-input name="name" label="Full Name" required value="" />

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-admin-form-input name="email" label="Email" type="email" required value="" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-admin-form-input name="outlet_id" label="Outlet" type="select" :options="['' => 'Select Outlet']" value="" />
